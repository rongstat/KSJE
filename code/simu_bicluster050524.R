...
for(pp in 1:length(sig.v)){
  ...
  for(NN in 1:100){
    ...
    #plot(umap(data.Y2),col=info2)
    #plot(umap(data.Y2[,1:6],spread=2),col=info2)
    ...
    #plot(umap(out.sepl,spread=2),col=info2)
    ...
    #error1[NN,1] = (rand.index(cls2[1:n],info2)+rand.index(cls1,info1))/2
    error2[NN,1] = rand.index(cls2[1:n],info2)
    #plot(umap(out.sepl,spread=2),col=info1)
