...
error=matrix(ncol=2,nrow=100)
error.out=matrix(ncol=2,nrow=20)
error2=matrix(ncol=2,nrow=100)
error.out2=matrix(ncol=2,nrow=20)
s.ind1 = matrix(ncol=2,nrow=100)
s.out1 = matrix(ncol=2,nrow=20)
s.ind2 = matrix(ncol=2,nrow=100)
s.out2 = matrix(ncol=2,nrow=20)
sig.v=seq(6,15, length.out= 20)
...
for(pp in 1:length(sig.v)){
  ...
  for(NN in 1:100){
    ...
    #colMeans(data.Y1)[1:6]
    #colMeans(data.Y2)[1:6]
    ...
    out.prop = prop.svd$v[,2:7] %*% diag(prop.svd$d[2:7])^(1/2)
    #plot(umap(out.prop))
    cls2=kmeans(out.prop,centers=6)$cluster
    s.ind2[NN,1] = mean(silhouette( info2, dist(out.prop[1:n,]))[,3])
    #plot(umap(out.prop),col=info2)
    out.prop = prop.svd$u[,2:7] %*% diag(prop.svd$d[2:7])^(1/2)
    cls1=kmeans(out.prop,centers=6)$cluster
    s.ind1[NN,1] = mean(silhouette( info1, dist(out.prop))[,3])
    error[NN,1] = (rand.index(cls2[1:n],info2)+rand.index(cls1,info1))/2
    error2[NN,1] = rand.index(cls1,info1)
    ...
    #plot(umap(out.prop))
    cls2=kmeans(out.prop,centers=6)$cluster
    s.ind2[NN,2] = mean(silhouette( info2, dist(out.prop[1:n,]))[,3])
    #plot(umap(out.prop),col=info2)
    out.prop = prop.svd$u[,2:5] %*% diag(prop.svd$d[2:5])^(1/2)
    cls1=kmeans(out.prop,centers=6)$cluster
    s.ind1[NN,2] = mean(silhouette( info1, dist(out.prop))[,3])
    error[NN,2] = rand.index(cls2[1:n],info2)
    error2[NN,2] = rand.index(cls1,info1)
    print(NN)
  }
...
data = data.frame(rand.index = c(s.out1),
                  method = factor(rep(c("Gamma1=Gamma2",
                                        "Gamma1!=Gamma2"), 
                                      each=length(sig.v))),
                  dim = rep(sig.v, times = 2))
...
ggplot(data, aes(x=dim, y=rand.index, group=factor(method), colour = method)) +
  ...
  geom_point(aes(shape=method) ,size=5) + ylab("silhouette index") + 
