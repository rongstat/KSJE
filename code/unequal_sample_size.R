...
for(pp in 1:length(n1.v)){
  ...
  for(NN in 1:100){
    ...
    n=sum(round(n1*c.prop))
    data1=rmvnorm(round(n1*c.prop[1]),mu1,sigma)
    data2=rmvnorm(round(n1*c.prop[2]),mu2,sigma)
    data3=rmvnorm(round(n1*c.prop[3]),mu3,sigma)
    data4=rmvnorm(round(n1*c.prop[4]),mu4,sigma)
    data5=rmvnorm(round(n1*c.prop[5]),mu5,sigma)
    data6=rmvnorm(round(n1*c.prop[6]),mu6,sigma)
    # data set X
    ...
    data.Y2[,(r):(r+20)] = data.Y2[,(r):(r+20)]+matrix(runif(n*21,-9*3,9),ncol=21) #noise in Y2
    info2=rep(1:6, times =  round(n1*c.prop))
...
ggplot(data, aes(x=dim, y=rand.index, group=factor(method), colour = method)) +
  geom_line(aes(linetype=method))+xlab("n2/n1")+
